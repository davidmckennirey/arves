...
1. ARVES is written in Python and is designed to be platform-independant (with the cavet that `masscan` doesn't run on windows)
...
- Install the required binaries
    - TODO: Add a list of requried binaries for the default config
    - TODO: Write a bash script to install the default binaries on *nix based systems
...
```bash
python3 arves.py -c config -o output -d hackerone.com
```

Running a full scan against a list of domains with a list of hostnames and IP addresses to include and exclude.
```bash
python3 arves.py -c config -o output -i include_list.txt -e exclude_list.txt -dL domain_list.txt
```
