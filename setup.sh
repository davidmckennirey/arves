...
if ! loc=$(command -v go)
then
    echo "Could not find go on the \$PATH.\nPlease install golang [apt install golang | brew install golang] and rerun this script."
    exit
else
...
then
    echo "Could not find aquatone on the \$PATH.\n.Aquatone has build issues so you have to download the correct version for your OS and put it in your \$PATH. (https://github.com/michenriksen/aquatone/releases/)"
    exit
else
...
then
    echo "Could not find massdns on the \$PATH.\n. Download and build massdns from its github repo. (https://github.com/blechschmidt/massdns)"
    exit
else
...
then
    echo "Could not find masscan on the \$PATH.\n. Download and build masscan from its github repo. (https://github.com/robertdavidgraham/masscan)"
    exit
else
...
then
    echo "Could not find nmap on the \$PATH.\n. There are like a million ways to install nmap, so I will let you figure that one out."
    exit
else
...
if test -f "./config/resolvers.txt"; then
    ...
    echo "No resolvers.txt file found in config directory. Generate a resolvers.txt file using the dnsvalidator project (https://github.com/vortexau/dnsvalidator) and save it to the config directory."
    exit
fi
...
if test -f "./config/all.txt"; then
    ...
    echo "No all.txt file found in config directory. Add a subdomain wordlist in your config directory called 'all.txt', I recommend the dns-Jhaddix.txt wordlist from SecLists (https://raw.githubusercontent.com/danielmiessler/SecLists/master/Discovery/DNS/dns-Jhaddix.txt)"
    exit
fi
